<template>
  ...
  <div :class="`${metric.classes} ${((metric_type_index === activeDetailsMetric.parentId && metric.id ===activeDetailsMetric.subId && $route.path ==='/details')) ? 'active':'bg-white'}`" class="stat card wrapper  max-w-xs inline  cursor-pointer rounded-2xl    hover:scale-2  relative" @click="$route.path='/details' ? setActiveCard(metric_type_index, metric.id): ''">
